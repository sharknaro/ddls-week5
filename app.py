...
async function load() {
  ...
  try {
    const response = await fetch('/api/umap'); if (!response.ok) throw new Error(`Data request failed (${response.status})`); umapData = (await response.json()).cells; if (!umapData.length) throw new Error('The dataset returned no cells.'); if (umapData.length !== 2700) throw new Error(`Expected 2,700 cells, but the dataset returned ${umapData.length}.`);
