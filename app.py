      ...
      <aside class="rounded-2xl border border-slate-800 bg-slate-900 p-4 shadow-xl">
        ...
        <div id="controls" class="space-y-4">
          ...
          <label class="block"><span class="text-sm text-slate-300">Show a gene on the UMAP</span>
            <div class="mt-1 flex gap-2"><input id="gene" list="genes" value="LST1" placeholder="e.g. FCER1G, LST1, CD3D" aria-label="Gene symbol" class="min-w-0 flex-1 rounded-lg border border-slate-700 bg-slate-800 px-3 py-2 uppercase" /><button id="geneBtn" class="rounded-lg bg-cyan-500 px-3 py-2 font-semibold text-slate-950">Show gene</button></div>
            <p class="mt-1 text-xs text-slate-500">Enter a gene symbol such as FCER1G, LST1, or CD3D to color the UMAP by its log-normalized expression.</p><datalist id="genes"></datalist>
          </label>
...
async function plotGene() { const gene=$('gene').value.trim().toUpperCase(); if(!gene){setStatus('Type a gene symbol before selecting Show gene.', 'error'); $('gene').focus(); return false;} const button=$('geneBtn'); setBusy(button, true, 'Show gene'); setStatus(`Loading ${gene} expression…`); try { const r=await fetch(`/api/genes/${encodeURIComponent(gene)}`); if(r.status===404) throw new Error(`Gene not found: ${gene} is not in this dataset. Try FCER1G, LST1, or CD3D.`); if(!r.ok) throw new Error(`Expression request failed (${r.status}).`); const payload=await r.json(); if(!payload.values?.length) throw new Error(`No expression values were returned for ${gene}.`); const byId=new Map(payload.values.map(d=>[d.cell_id,d.expression])); Plotly.newPlot('plot',[{x:umapData.map(d=>d.x),y:umapData.map(d=>d.y),mode:'markers',type:'scattergl',name:`${gene} expression (log-normalized)`,text:umapData.map(d=>`${d.cell_id}<br>${gene} (log-normalized): ${byId.get(d.cell_id).toFixed(3)}`),hoverinfo:'text',marker:{color:umapData.map(d=>byId.get(d.cell_id)),colorscale:'Cividis',size:6,line:{color:'#0f172a',width:.2},colorbar:{title:`${gene} (log-normalized)`,titlefont:{color:'#f8fafc'},tickfont:{color:'#cbd5e1'}}}}],layout(`UMAP — ${gene} expression (log-normalized)`),{responsive:true,displaylogo:false}); setGeneView(gene); setStatus(`${gene} loaded. Values are log-normalized expression from adata.X.`, 'success'); return true; } catch(error) { setStatus(error.message, 'error'); return false; } finally { setBusy(button, false, 'Show gene'); } }
