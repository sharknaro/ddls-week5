...
async function load() {
  ...
  try {
    ...
    $('cluster').addEventListener('change', () => { loadMarkers(); if (activeView !== 'cluster') draw(); });
    $('markCluster').addEventListener('click', () => { markCluster = !markCluster; draw(); });
...
function draw() {
  ...
  $('selectedClusterSection').style.display = mode === 'gene' ? 'none' : '';
